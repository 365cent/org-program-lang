...
def is_inside_string(line, index):
    ...
    # print(line, single_quote_count % 2 != 0 or double_quote_count % 2 != 0)
...
def rm(file_path):
    ...
    for j, line in enumerate(lines):
        ...
        if len(temp_lines) != 0 and not stripped_line.endswith("\\"):
            # print(temp_lines)
            stripped_line = ''.join(temp_lines) + stripped_line
            temp_lines = []
        elif stripped_line.endswith("\\"):
            temp_lines.append(stripped_line[:-1])
            continue
        ...
        if inside_multiline_string:
            ...
            if stripped_line.startswith("'''") or stripped_line.startswith('"""'):
                if (stripped_line.count("'''") == 2 or stripped_line.count('"""') == 2) and len(stripped_line) > 3:
                    ...
                    continue
            elif stripped_line.endswith(("'''", '"""')) and is_inside_string(stripped_line, len(stripped_line) - 3):
                inside_multiline_string = True
